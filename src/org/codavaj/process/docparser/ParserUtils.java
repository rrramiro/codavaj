...
public class ParserUtils extends AbstractLogger {
    ...
    public String javadocLinkToTypename(String link, List externalLinks) throws UnresolvedExternalLinkException {
        ...
        if ( link.startsWith("http:") || link.startsWith("https:")) {
            ...
            if ( link.startsWith("http://java.sun.com/") && link.indexOf("/api/") != -1 ) {
                ...
                link = link.substring(link.indexOf("/api/") + "/api/".length()).replaceAll(".html.is-external=true", "");
            } else {
                boolean found = false;
    ...
    public void determineTypeModifiers(Type type, Document typeXml, List externalLinks) {
        ...
        */
    	...
    	Node typeDescriptorElement = typeXml.selectSingleNode("//DL[preceding-sibling::H2 and ./DT[string-length(.) > 0 and not(contains(.,'All')) and not(contains(.,'Enclosing')) and not(contains(.,'Direct')) and not(contains(.,'Type Parameters:'))]]");
        if(typeDescriptorElement == null){
            typeDescriptorElement = typeXml.selectSingleNode("//DL[parent::P/preceding-sibling::H2 and ./DT[string-length(.) > 0 and not(contains(.,'All')) and not(contains(.,'Enclosing')) and not(contains(.,'Direct')) and not(contains(.,'Type Parameters:'))]]");
            System.out.println("####################");
            System.out.println(typeDescriptorElement.asXML());
            System.out.println("---------------------");
        }

    	String typeDescriptor = convertNodesToString((Element) typeDescriptorElement, externalLinks);
